...
import dataCases from "@/app/dataCases";
...
export default async function singleCases({ params }) {
  const { slug } = await params;
  const response = await fetch(`https://jhgcdsutaztszxtyehtm.supabase.co/rest/v1/cases?slug=eq.${slug}`, {
    method: "GET",
    headers: {
  ...
  const data = res[0];
  const cases = dataCases.filter((item) => item.id === data.id);
    ...
    <section className={`${work_header.className} text-red uppercase  text-headers relative`}>
      <div className="relative overflow-hidden w-screen">
        ...
        <div className="relative flex animate-program-2 w-screen">
          <p className="text-running flex-shrink-0 tracking-widest word-spacing-custom py-3">cases cases cases cases cases cases cases cases cases cases cases cases cases cases cases cases</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-screen px-0 pt-20 work_bread">
        <div className="md:col-start-1 md:row-start-1 relative w-full pb-[100%] overflow-hidden mt-3">
          <Image src={cases[0].images[0].src} alt={cases[0].images[0].alt || "Default alt text"} layout="fill" objectFit="cover" />
        </div>
        ...
        {cases[0].images.slice(1).map((image, i) => (
